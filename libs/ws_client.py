...
def generateLocalId(paramInt=8):
    intRange = range(1, paramInt+1)
    arrayList = []
    for i in intRange:
        arrayList.append(random.choice("abcdefghijklmnopqrstuvwxyz0123456789"))
    strn = ''.join(arrayList)
    stringBuilder = strn + str(now_ms())
...
def KICK(ws_buffer, data):

	return {"type": "kick", "chat": data.get("chat_name")}
...
class Buffer:
	...
	def __init__(self, bearer, user_id, username, region, callback):
		...
		self.headers = {"Sec-WebSocket-Protocol": "wamp.json","Sec-WebSocket-Version": "13","Sec-WebSocket-Key": "eXp6ZmVxandraW13eG54bw==","Cookie":"_ga_5FQ9GH4QMZ=GS1.1.1711757771.1.1.1711760137.0.0.0; _ga=GA1.1.1267191589.1711757772; _gcl_au=1.1.660210167.1711757770; device_id=b1c7921b276ce23f914b48637ea94e6e3bbd2e3e532057e8e4aeaa4e25bf6226"}
	...
	async def ping_ifunny(self):
		await self.ifunny_ws.send(json.dumps([32,self.ifunny_ws_counter,{},f"co.fun.chat.user.{self.user_id}.chats"]))
		await self.ifunny_ws.send(json.dumps([32,self.ifunny_ws_counter,{},f"co.fun.chat.user.{self.user_id}.invites"]))
		#print("Pinged")
	...
	async def form_client_frame(self, frame):
		...
		if frame_type_id in (3, 6): #authentication error/shutdown
			...
			cprint(("Authenticated as", "magenta"), (f"{self.user_id} {self.username}", "yellow"),)
			...
			elif frame_data["type"] == 101: #101 is you being removed from a chat (Kicked)
				client_frame = KICK(self, frame_data)
	...
	async def form_ifunny_frame(self, frame):
		...
		if frame_type == "message":
			...
			payload["localId"] = generateLocalId()
		...
		elif frame_type == "join_chat":

			return json.dumps(
				[48,self.ifunny_ws_counter,{},
	 			"co.fun.chat.join_chat",[],{"chat_name":chat_id}])
			...
			return json.dumps([48,self.ifunny_ws_counter,{},"co.fun.chat.get_chat",[],{"chat_name":chat_id}])
		
		elif frame_type == "read_chat":
