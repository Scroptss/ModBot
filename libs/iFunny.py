...
class Parser:
	...
	async def message(bot, ctx, frame):
		...
		ctx.author.is_developer = ctx.author.id in bot.developer				
	...
	@staticmethod
	async def kick(bot,ctx,frame):

		ctx.chat = Chat({'id':frame['chat']}, bot)
		await bot.siphon_input(bot.on_kick, ctx)
	...
	async def file(bot, ctx, frame):
		...
		if user_id == bot.user_id: return
		if frame["user"].get("is_bot"): return
		if user_id in bot.blacklist() and user_id not in bot.developer: return
...
class CTX:
	...
	async def user_by_nick(self, nick: str):
		return await user_by_nick(nick, self.bot)
		
	async def user_by_id(self, user_id: str):
		return await user_by_id(user_id, self.bot)
...
class User(CTXtype):
	...
	def __init__(self, data, bot):
		...
		self.developer = self.id in bot.developer
	...
	async def send(self, message):
		await self.bot.get_or_create_chat(self.chat_id)
...
class Chat(CTXtype):
	...
	async def share(self, message):
		...
		author_name = None
		if self.author and self.type != 1: author_name = self.author.nick
		await self.bot.share_message(self.id, message, author_name)
	...
	async def invite(self, user, chat=None):
		if isinstance(user,User):
			await self.bot.invite(self.id, user.id)
		else: await self.bot.invite(chat, user)

	async def register_operator(self, user):
		await self.bot.add_operator(self.id, user.id)

	async def unregister_operator(self, user):
		await self.bot.remove_operator(self.id, user.id)

	async def kick(self, user):
		if isinstance(user,User):
			return await self.bot.kick(self.id, user.id)
		await self.bot.kick(self.id,user)
		
	async def input(self, type=Message, timeout=None):
		return await self.bot.input(self.id, type, timeout)
	
	async def check_invite(self,author,user):
		return await self.bot.check_invite(author, user)

class Bot:
	
	def __init__(self, email: str, password: str, region: str, prefix: str, developer: list):
	...
	def login(self):
		...
		while True:
			...
			if self.bearer:
				self.buff = ws_client.Buffer(self.bearer, self.user_id, self.username, self.ws_region, self.parse)
				return	
		...
		acctheader = {"Host":"api.ifunny.mobi","Accept":"video/mp4, image/jpeg","Applicationstate":"1","Ifunny-Project-Id":"iFunny",'User-Agent': 'iFunny/8.41.11(24194) iPhone/16.3.1 (Apple; iPhone12,5)',"Accept-Language": "en-US;q=1","Authorization":"Bearer " + self.bearer}
		Account = requests.get(host + "/v4/account", headers = acctheader).json()
		print(Account)
		self.user_id = Account["data"]["id"]
		self.username = Account["data"]["original_nick"]
	...
	def command(self, *args, **kwargs):
		def container(function):
			...
			if hide_cooldown := kwargs.get("hide_cooldown"):
				self.hidden_cooldowns[function] = hide_cooldown
	...
	def event(self, *args, **kwargs):
		def container(function):
			...
			valid_types = ("user_join", "user_leave", "user_kick", "channel_change", "on_join", "on_kick", "on_message", "on_file")
	...
	async def share_message(self, chat_id, message, nick=None):
	
		chunks = textwrap.wrap(str(message), 500, break_long_words=True, replace_whitespace=False)
		
		for message in chunks:
			await self.buff.send_ifunny_ws(json.dumps([48,self.buff.ifunny_ws_counter,{},"co.fun.chat.share_message",[],{"users":[],"text":message,"chats":[chat_id]}]))
	...
	async def check_invite(self,author,user):
		...
		privacy = requests.get(f"https://api.ifunny.mobi/v4/users/{user.id}",headers=headers).json()
		status = privacy["data"]["messaging_privacy_status"]
		if status == "public":
			return 0
		if status == "closed":
			return 1
		...
		while True:
			...
			if next: params = (('limit','100'),('next',next),)			
			response = requests.get(f'https://api.ifunny.mobi/v4/users/{user.id}/subscriptions', headers=headers, params=params).json()
			...
			if paging.get("hasNext"):
				next = paging["cursors"]["next"]

			for sub in subscriptions:
				if sub["id"] == author.id:
					return 0

			if not next: return 2
	...
	async def read_chat(self,ctx):
		await self.buff.send_ifunny_ws(await self.buff.form_ifunny_frame({"type": "read_chat", "chat_id": ctx.chat.id}))
	...
	async def get_or_create_chat(self,chat_id):
		request_id = self.buff.ifunny_ws_counter
		self.chat_request_ids[request_id] = chat_id

		self.chat_list_queues[chat_id] = asyncio.Queue()
		await self.buff.send_ifunny_ws(json.dumps([48,self.buff.ifunny_ws_counter,{},"co.fun.chat.get_or_create_chat",[1,chat_id,None,None,None,[self.user_id]],{}]))
	...
	async def get_members(self, chat_id):
		...
		try:
			member_list = await asyncio.wait_for(self.member_list_queues[chat_id].get(), 10)
		except asyncio.TimeoutError:
			member_list = []
	...
	async def add_operator(self, chat_id, user_id):
		await self.buff.send_ifunny_ws(json.dumps([48,self.buff.ifunny_ws_counter,{},"co.fun.chat.register_operators",[],{"chat_name":f"{chat_id}","operator_ids":[f"{user_id}"]}]))


	async def remove_operator(self, chat_id, user_id):
		await self.buff.send_ifunny_ws(json.dumps([48,self.buff.ifunny_ws_counter,{},"co.fun.chat.unregister_operators",[],{"chat_name":f"{chat_id}","operator_ids":[f"{user_id}"]}]))
	...
	async def join(self, ctx, chat):
		await self.buff.send_ifunny_ws(await self.buff.form_ifunny_frame({"type": "join_chat", "user_id": self.user_id, "chat_id": chat.name}))
		if self.on_join:
			await asyncio.sleep(0.1)
			self.run_callback(self.on_join, ctx)
	...
	async def run_command(self, function, ctx):
		...
		hidden = self.hidden_cooldowns.get(function)
		now = time.time()
				...
				if user_timekeeping:
					...
					if ratelimit_expires_at:
						if now < ratelimit_expires_at:
							...
							remaining_time_str = self.seconds_to_str(remaining_time)
							if not hidden:
								await ctx.chat.send(f"You must wait {remaining_time_str} before you can use this command again")
							return
						else:
